---
title: "Trends of Road Traffic Accidents"
...
::: callout-note
This blog utilizes accident data sourced from daily newspapers. It does not aim to substitute or dispute official police traffic reports. Instead, it aims to complement the existing narrative by providing additional context to the road traffic accident reports disseminated by the media. As such, any interpretation of these findings should be done with this context in mind.This monthly blog therefore seeks to provide patterns of major accidents on highways, given that the majority of such incidents are covered by the media
:::
...
pacman::p_load(rio, 
               ...
               wesanderson,
               hrbrthemes)
df <- read.csv("C:/Users/deurien/Dropbox/MBA/Company/Accident blog post/Project/Blog_project/data/final_clean_data.csv")
#clean names--------------------------------------------------------------------
df_clean <- df |>
  clean_names()
...
The figure below indicates the availability of variables of interest as reported in the newspapers. The variables most reported by the newspaper articles include;

1.  Village of accident
...
ggplot(plot_data_long, aes(x = fct_reorder(variable, proportion), y = proportion, fill = variable, label = proportion)) +
  ...
  geom_text(vjust = -0.1, hjust = -0.2, size = 4, color = "darkred") +
  labs(x = "Variable", 
  ...
  guides(fill = FALSE)+
  theme(
  plot.title = element_text(face = "bold", size = 18, color = 'royalblue4'), 
  plot.subtitle = element_text(face = "bold", size = 14, color = 'royalblue'), 
  axis.text.x = element_text(hjust = 0, size = 12, face = 'bold'),  
...
## Trend of Total Accidents Reported in Newspapers, 2021-2023,

Nwespapers reported an average of 96 road crushes on highways over the 3 years under study.
...
ggplot(accidents_by_year, aes(x = factor(year), y = total_accidents, fill = factor(year), label = total_accidents)) +
  ...
  labs(x = "Year", y = "Number of Road Accidents", title = "Overall Number of Accidents Reported in Daily Newspapers, 2021-2023.", size = 14) +
...
There is an overall spike in the number of accidents were reported in May followed by November to December period.
...
ggplot(accidents_by_month, aes(x = month, y = total_accidents)) +
  ...
  labs(x = "Month", y = "Total Accidents", title = "Total Accidents by Month and Year") +
...
There is a consistent marginal increase in road crushes reported by newspapers in May in all the 3 years under study.
...
ggplot(accidents_cars, aes(x = month, y = factor(cars), fill = total_accidents)) + 
  ...
  labs(x = "Month", y = "Cars", title = "Total Accidents by Month and Cars") +
  scale_x_continuous(breaks = 1:12, labels = month.abb) +  # Set x-axis to show months
  theme_light()
```

## Total Number of Accidents Reported by Month by Type of Car Involved Faceted by Year

Trucks registered the highest number of accidents consistent reported by newspapers over the 12 months of year in all the 3 years of study.
...
## Total Number of Accidents Reported by Time of Occurence
...
ggplot(accidents_cars_at, aes(x = time, y = factor(type_of_accident), fill = total_accidents)) + 
  geom_tile() +
  scale_fill_gradient(low = "#FCA594", high = "#AE123A", na.value = "pink") +  # Adjust the color gradient
  labs(x = "Month(Hours)", y = "Cars", title = "Total Accidents by Month and Cars") +
...
Generally, most accidents happened at 6 am in the morning. Cars overturning was the most type of accident reported at 6 am over the 3 years under study.
...
ggplot(accidents_cars_at_yr, aes(x = time, y = factor(type_of_accident), fill = total_accidents)) + 
  geom_tile() +
  scale_fill_gradient(low = "#FCA594", high = "#AE123A", na.value = "pink") +  # Adjust the color gradient
  labs(x = "Time(Hours)", y = "Cars", title = "Total Accidents by Month and Cars") +
  scale_x_continuous(breaks = 0:23) +  # Set x-axis to show months
  theme_light()+
  facet_wrap(~ year, ncol = 1)+
  geom_vline(xintercept = 6, linetype = "dashed", color = "blue", size = 0.8,
...
#Create day of week-------------------------------------------------------------
df_cleann <- df_clean %>%
  mutate(day_of_week = weekdays(as.Date(publish_date)))
...
ggplot(accidents_day_time, aes(x = time, y = day_of_week, fill = total_accidents)) +
  ...
  labs(
    ...
    y = "Day of Week",
    title = "Heatmap of Accidents by Day of Week and Time",
...
# Extract year from 'publish_date'
df_cleann <- df_cleann %>%
  mutate(year = year(as.Date(publish_date)))
...
The prevalent occurrence of road crushes at 6 am in the morning is consistent over the three years under study. However, the consistency in days is only true on Monday mornings and spread ramdomly throught the days of the week when compared over the three years.
...
1.  The average number of road crashes reported in newspapers over the three-year period was 96.

2.  May and the November to December period showed a higher number of reported accidents.

3.  Trucks consistently had the highest number of reported accidents throughout the year.

4.  The majority of accidents reported occurred at 6 am in the morning.

5.  Cars overturning was the most common type of accident reported at 6 am.

# Recommendations for further study

1.  Analyze the underlying causes and contributing factors for the spike in accidents during May and the November to December period.

2.  Investigate the reasons behind the consistent high number of accidents involving trucks and explore potential measures to reduce their occurrence.

3.  Examine the circumstances surrounding accidents occurring at 6 am to identify specific risk factors and develop targeted interventions.

4.  Explore the patterns of head-on collisions and cars ramming into stationary trucks at night to understand the factors contributing to these types of accidents.

5.  Investigate the reasons for the higher number of accidents reported on Mondays and Tuesdays, as well as the lower number on Saturdays, to identify potential factors influencing these trends.
